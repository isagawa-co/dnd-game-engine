...
- **On exit:** consume the item from campaign-scoped storage (`campaign_state`/`party_toolkit` consumables — NOT the shared sheet), apply the effect to `campaign_state` (e.g., `current_hp`), hand control back to scene.

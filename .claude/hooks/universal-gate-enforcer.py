...
def get_actions_count() -> int:
    """Count actions from the JSONL log file (authoritative action count source)."""
...
def main():
    ...
    if tool_name == 'Bash':
        ...
        # ENFORCEMENT: Block foreground bash with cd prefix (RULE ZERO violation)
        run_in_background = tool_input.get('run_in_background', False)
        if not run_in_background:
            cmd_stripped = command.strip().lower()
            if cmd_stripped.startswith('cd ') or cmd_stripped.startswith('cd\t'):
                sys.stderr.write("""BLOCKED: Foreground bash cannot use cd

Rule: cd in foreground bash shifts cwd for the session and breaks hook path resolution.
See: .claude/lessons/lessons.md → RULE ZERO

FIX:
1. Use absolute paths directly in your command instead of cd
2. If you need to work in another directory, use run_in_background: true (safe — runs in subprocess)

Examples:
❌ Bad:  Bash(command: 'cd /repo && python3 main.py')
✓ Good: Bash(command: 'python3 /repo/main.py')
✓ Good: Bash(command: 'cd /repo && python3 main.py', run_in_background: true)
""")
                sys.exit(2)
    ...
    # One-shot agents (spawned by run-task.sh) skip anchor, counter, token, and hash gates.
    # They execute one task and exit — anchor drift isn't a risk for single-task agents.
    # Gates 1 (session) and 2 (learn) still apply to one-shot agents.
    is_one_shot = session_state.get('one_shot', False)

    # Gate checks — safe bash skips these (never blocked) but still increments below
    if not safe_bash:
        # Gate 1: Session started? (applies to ALL agents including one-shot)
        ...
        # Gate 2: Needs learn? (applies to ALL agents including one-shot)
        ...
        # Gate 5: Anchor token confirmed? (skip for one-shot agents)
        if not is_one_shot:
